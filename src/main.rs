...
use wasmer::{
    imports, sys::EngineBuilder, sys::Features, wat2wasm, AsStoreMut, AsStoreRef, Function,
    FunctionEnvMut, Instance, Memory, Module, Store, Value,
};
use wasmer::{CompilerConfig, FunctionEnv, FunctionType, Type};
...
"#
    ...
    let engine = EngineBuilder::new(compiler_config).set_features(Some(features));

    let mut store = Store::new(engine);
    let module = Module::new(&store, contract_bytecode).unwrap();
    ...
    let seed_typed = Function::new_typed_with_env(&mut store, &env, seed);
    ...
    let __new = instance.exports.get_function("__new").unwrap();
    let __pin = instance.exports.get_function("__pin").unwrap();
    let memory = instance.exports.get_memory("memory").unwrap();

    let data: i32 = lower_string("Hello, World!", __new, __pin, memory, &mut store).unwrap() as i32;

    match init.call(&mut store, &[Value::I32(data), Value::I32(data)]) {
...
fn lower_string(
    ...
    let result = __new.call(
        ...
        &[Value::I32((length << 1) as i32), Value::I32(2)],
    )?;
...
fn get_op_cost(operator: &Operator) -> u64 {
    use Operator::*;
