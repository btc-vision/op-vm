...
impl HugePageLinearMemory {
    ...
    ) -> Result<(*mut u8, bool), MemoryError> {
        use libc::{
    ...
    fn allocate_memory(
        ...
        let layout = std::alloc::Layout::from_size_align(total_size, REGULAR_PAGE_SIZE)
...
impl LinearMemory for HugePageLinearMemory {
    ...
    fn grow(&mut self, delta: Pages) -> Result<Pages, MemoryError> {
        ...
        if new_bytes > max_bytes {
            ...
            let (new_ptr, new_uses_huge) =
                Self::allocate_memory(new_total, new_bytes, &self.huge_page_config)?;
            ...
            // Just make more memory accessible
            #[cfg(target_os = "linux")]
            {
